...
const userController = {
    login: [
        ...
        async (request, response) => {
            try {
                ...
                response.json({ message: "Usuario logeado correctamente", token: token });
            } catch (error) {
    ...
    register: [
        ...
        async (request, response) => {
            try {
                ...
                response.cookie('token',token);
                response.status(201).json({ message: "Usuario registrado correctamente", token: token });
            } catch (error) {
    ...
    avatar: [
        async(request, response) => {
            ...
            if(!avatar) return response.status(400).json({message: 'Falta la imagen'});
            try {
                ...
                response.json({message: 'Imagen actualizada', avatar: updateUser.avatar});
            } catch (error) {
